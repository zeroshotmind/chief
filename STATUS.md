...
|---|---|---|---|
| 1 | REST API for all core operations | Built | 60 routes under `/v1`; `api/routes.py` |
| 2 | MCP interface covering the same operations | Built | 30 tools against 60 routes, `mcp_server.py`, HTTP on the same app. Surface reconciled in MCP-SURFACE.md; `tests/test_transport_parity.py` holds the two apart from drifting |
...
60 routes: the seven update/instance routes are three path-parameterised service methods, so
...
60 routes, also served without the `/v1` prefix; `/healthz` is not counted here. Full
